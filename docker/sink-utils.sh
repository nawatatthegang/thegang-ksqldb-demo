...
if [[ $1 == "get" ]]; then
  ...
  curl -X POST -H "Content-Type:application/json" -d "@$2" http://localhost:8084/connectors 2>/dev/null | jq
elif [[ $1 == "update" ]]; then
  curl -X PUT -H "Content-Type:application/json" -d "$(jq '.config' $3)" "http://localhost:8084/connectors/$2/config" 2>/dev/null | jq
elif [[ $1 == "delete" ]]; then
