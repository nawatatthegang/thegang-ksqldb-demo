...
if [[ $1 == "get" ]]; then
  curl "http://localhost:8083/connectors/$2" 2>/dev/null | jq
elif [[ $1 == "status" ]]; then
  ...
  curl -X PUT "http://localhost:8083/connectors/$2/pause" 2>/dev/null
elif [[ $1 == "resume" ]]; then
  curl -X PUT "http://localhost:8083/connectors/$2/resume" 2>/dev/null

elif [[ $1 == "stop" ]]; then
  curl -X PUT "http://localhost:8083/connectors/$2/stop" 2>/dev/null
elif [[ $1 == "restart" ]]; then
  curl -X POST "http://localhost:8083/connectors/$2/restart?includeTasks=true" 2>/dev/null
fi
